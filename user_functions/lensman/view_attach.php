	...
	<?php 
	$posts = $conn->query("SELECT p.* , u.user_profile_image, u.user_name from tbl_post p inner join tbl_user_account u on u.user_id = p.user_id  where p.post_id = {$_GET['id']}");
	...
	$is_liked =  $conn->query("SELECT * FROM tbl_post_likes where user_id = {$_SESSION['login_user_id']} and post_id = {$_GET['id']} ")->num_rows ? "text-primary" : "";
	$liked =  $conn->query("SELECT * FROM tbl_post_likes where post_id = {$_GET['id']} ")->num_rows;
	$commented =  $conn->query("SELECT * FROM tbl_post_comments where post_id = {$_GET['id']} ")->num_rows;
	?>
	<style>
			...
			<div class="card card-widget post-card" data-id="<?php echo $post_id ?>">
				<div class="card-header">
					<div class="user-block w-100">
						<img class="img-circle" src="../../<?php echo $user_profile_image ?>" alt="User Image">
						<span class="username"><a href="#"><?php echo $user_name ?></a></span>
						<span class="description">Posted - <?php echo date("M d,Y h:i a",strtotime($post_date)) ?></span>
					</div>
				</div>
				<div class="card-body">
				...
				<div class="card-footer card-comments">
					<?php 
						$comments = $conn->query("SELECT c.*, u.user_name,u.user_profile_image FROM tbl_post_comments c inner join tbl_user_account u on u.user_id = c.user_id where c.post_id = {$_GET['id']} order by unix_timestamp(c.comment_date) asc ");
