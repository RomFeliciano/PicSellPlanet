	...
	Class Action {
		...
		function save_comment(){
			...
			if($save){
				$id = $this->db->insert_id;
				$d['status'] = 1;
				$qry = $this->db->query("SELECT c.*, u.user_name,u.user_profile_image FROM tbl_post_comments c inner join tbl_user_account u on u.user_id = c.user_id where c.comment_id = $id ")->fetch_array();
				foreach($qry as $k => $v){
					if(!is_numeric($k)){
